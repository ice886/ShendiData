    ...
    async def generate_qa_pairs(self, output_dir: str = None):
        ...
        for i, img_info in enumerate(self.images, 1):
            ...
            if caption_text:
                ...
                system_prompt = f"""你是一个专业的学术论文视觉理解助手。请严格分析图片并生成 1 个高质量问答对。

                                📊 图片类型识别指南：
                                - 数据图表：提取坐标轴含义、峰值/谷值、趋势、图例
                                - 实验结果：关注对照组差异、显著性标注、误差范围
                                - 示意图/流程图：提取核心模块、数据流向、关键步骤
                                - 其他：客观描述可见元素与学术关联

                                ⚠️ 严格约束：
                                1. 问题必须具体到图片中的某一数据、趋势或结构，禁止“这张图讲了什么”
                                2. 回答必须包含图片中可验证的关键信息，禁止编造数据
                                3. 使用中文，学术严谨，长度 80~250 字
                                4. 仅输出合法 JSON，禁止 Markdown、解释性文字或多余符号

                                📦 输出格式：
                                ```json
                                {{
                                "question": "基于图片具体元素提出的问题",
                                "answer": "包含可验证数据/趋势的详细回答"
                                }}
                                ```
                            """  
